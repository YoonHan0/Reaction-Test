      ...
      <Layout>
        <div className="flex flex-col items-center gap-4 py-10 text-center text-white">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-400">익명 로그인 상태를 확인하고 있습니다.</p>
          </div>
        </div>
      </Layout>
    );
        ...
        <div className="flex flex-col gap-6 text-white">
          ...
          <div className="rounded-2xl border border-slate-700/80 bg-slate-800/50 p-4">
            <div className="mb-4 space-y-1">
              <p className="text-sm font-medium text-slate-200">Turnstile 인증</p>
              <p className="text-xs text-slate-400">검증 완료 후 자동으로 익명 로그인을 시도합니다.</p>
            </div>
            ...
            {error && (
              <div className="mt-4 flex flex-col gap-3 rounded-xl border border-red-500/20 bg-red-500/10 p-3">
                <p className="text-sm text-red-300">{error}</p>
                <button
                  type="button"
                  onClick={retry}
                  disabled={isSigningIn}
                  className="inline-flex w-fit items-center gap-2 rounded-lg border border-white/10 bg-white/10 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-50"
                >
